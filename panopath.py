...
def crawl(url):
    ...
    for result in results:
        link=result.find("a")
        if link:
            temp=result.get_text().strip().encode('ascii', 'ignore').decode('ascii')
            if temp:
                uni.append(temp)
                links.append(link.get("href")) 
    mismatched = soup.find('h3', attrs={'style': 'margin-bottom: 5px;list-style: none;font-size: medium;line-height: 1.54;color: rgb(51, 51, 51);font-family: arial;white-space: normal;text-align: left;background-color: rgb(255, 255, 255);'})
    temp = mismatched.text.encode('ascii', 'ignore').decode('ascii')
    uni.append(temp)
    links.append(mismatched.find("a").get("href"))
crawl('https://mp.weixin.qq.com/s/-PPUaNCMF3IZdaJnlUtsSw?')
uni=uni[1:82]+[uni[160]]
links=links[1:82]+[links[160]]
panopath=["DNE"]*len(universities)
#String manipulation to correct misspellings in the items crawled
for x in range(len(uni)):
    ...
    if uni[x][0]==" ":
        uni[x]=uni[x][1:]
    uni[x]=uni[x].replace("-","--")
    uni[x]=uni[x].replace(" in ","--")
    uni[x]=uni[x].replace(" at ","--")
    uni[x]=uni[x].replace("&","and")
    uni[x]=uni[x].replace(", ","--")
    uni[x]=uni[x].replace("South","Southern")
    uni[x]=uni[x].replace("Williams","William")
    uni[x]=uni[x].replace("Institution","Institute")
    uni[x]=uni[x].replace("Brandies","Brandeis")
    uni[x]=uni[x].replace("Univeristy","University")
    uni[x]=uni[x].replace("Case Western","Case Western Reserve")
    uni[x]=uni[x].replace("Purde","Purdue")
    uni[x]=uni[x].replace("ana--C","ana-C")
    uni[x]=uni[x].replace (" Bloomington","--Bloomington")
    uni[x]=uni[x].replace (" Amherst","--Amherst")
    uni[x]=uni[x].replace("Notre Dame University","University of Notre Dame")
    uni[x]=uni[x].replace("--St"," in St")
    for i in range(len(universities)):
        if uni[x] in universities[i] or universities[i] in uni[x]:
            panopath[i]=links[x]
            break #prevents matching with more than one university
big=[universities,links]
big=list(map(list, zip(*big)))
with open('Panopath.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(["Name","Panopath Link"])
