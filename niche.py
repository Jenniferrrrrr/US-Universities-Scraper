...
niche,division,acceptance,undergrad,graduation,url =[],[],[],[],[],[]
def crawl(url):
    ...
    def check(link):
        nonlocal found
        if ('https://www.niche.com/colleges/' in link) and ('imgre' not in link):
            found = True
            niche.append(link)
        return found
...
for uni in universities:
    crawl('https://www.google.com/search?q='+urllib.parse.quote(uni)+"+niche")

for link in niche:
    if link =="DNE":
        division.append("DNE")
        acceptance.append("DNE")
        undergrad.append("DNE")
        graduation.append("DNE")
        url.append("DNE")
    else:
        req = requests.get(link,headers=headers)
        html = req.content
        soup = BeautifulSoup(html, 'lxml')
        result= soup.find('div', attrs={'class': 'scalar--two'}).find('div', attrs={'class': 'scalar__value'})
        if result:
            division.append(result.find("span").text)
        else:
            division.append("DNE")
        results = soup.find_all("div",attrs={'class': 'scalar'})
        for i in range(len(results)):
            x= results[i].find_all("span")
            if len(x)==2:
                if (x[0].get_text().strip()=="Acceptance Rate"):
                    acceptance.append(x[1].text)
            # elif i == len(results)-1:
            #     acceptance.append("DNE")
        result = soup.find("a",attrs={'class':"profile__website__link"}).get("href")
        url.append(result)
        results = soup.find_all("div",attrs={'class':"scalar__value"})
        for result in results:
            search =result.find("span",attrs={'class':"scalar__value__suffix"})
            if search and search.text=="Undergrads":
                undergrad.append(result.find("span").text)
        results= soup.find_all("div",attrs={'class': "scalar--three"})
        for result in results:
            search =result.find("div",attrs={'class':"scalar__label"}).find("span")
            if search and search.text=="Graduation Rate":
                graduation.append(result.find("div",attrs={'class': 'scalar__value'}).find("span").text)

big=[universities,niche,division,acceptance,undergrad,graduation,url]
big=list(map(list, zip(*big)))
with open('Niche.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(["Names","Niche Link","Atheletic Division","Acceptance Rate","Undergrad #","Graduation Rate","School Website"])
